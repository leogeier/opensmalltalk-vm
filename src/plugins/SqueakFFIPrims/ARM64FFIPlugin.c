 ...
 */
static char __buildInfo[] = "ThreadedARM64FFIPlugin VMMaker.oscog-eem.3726 uuid: df260b88-5339-458c-9731-cb84179ff32a " __DATE__ ;
...
#define PluginVersionInfo " VMMaker.oscog-eem.3726"
...
static const char *moduleName = "ARM64FFIPlugin VMMaker.oscog-eem.3726 " INT_EXT;
static sqInt nilObj;
...
l3:
		...
		/* We test the 2nd inst var to distinguish between an ExternalFunction and an ExternalVariable.
		   ...
		   ExternalVariable allInstVarNames. #('handle' 'type' 'name' 'module') */
		if (isIntegerObject(fetchPointerofObject(1, errorReaper))) {
			if (!addressSqInt) {
			...
			retSpec = fetchPointerofObject(0, argTypeArraySqInt);
			...
			retSpec = fetchPointerofObject(1, errorReaper);
 ...
 || (isCharacterAtomicType((calloutState->errorResultType)))))));
	...
	if (atomicType <= FFITypeSignedInt32) {
		...
		if (atomicType <= FFITypeSignedInt32) {
			...
			value = intRet & ((((((unsigned long long)1)) << shiftSqInt)) - 1);
			if (((atomicType & 1) != 0)) {
				/* make the guy signed */
