/* Automatically generated by
	VMPluginCodeGenerator VMMaker.oscog-eem.3553 uuid: 621c3ab3-1916-49cc-942c-901c1e0c4660
	(Compiler-eem.514)
   from
	IA32ABIPlugin VMMaker.oscog-eem.3553 uuid: 621c3ab3-1916-49cc-942c-901c1e0c4660
 */
static char __buildInfo[] = "IA32ABIPlugin VMMaker.oscog-eem.3553 uuid: 621c3ab3-1916-49cc-942c-901c1e0c4660 " __DATE__ ;
...
static const char *moduleName = "IA32ABI VMMaker.oscog-eem.3553 " INT_EXT;
...
indexlengthinRange(sqInt byteIndex, sqInt length, sqInt rcvr)
{
	...
	dataSize = longAt((void *)(rcvr + BaseHeaderSize));
	return (dataSize == 0)
	 || ((byteIndex >= 0)
...
primAddressField(void)
{
	...
	rcvr = stackValue(0);
	value = longAt((void *)((rcvr + BaseHeaderSize) + BytesPerOop));
...
primAddressFieldPut(void)
{
	...
	longAtput((void *)((rcvr + BaseHeaderSize) + BytesPerOop),value);
...
primAlienCopyInto(void)
{
	...
	myLength = longAt((void *)(alien + BaseHeaderSize));
	src = ((/* startOfData:withSize: */
	(myLength > 0
		? (alien + BaseHeaderSize) + BytesPerOop
		: longAt((void *)((alien + BaseHeaderSize) + BytesPerOop)))) + start) - 1;
	if (isKindOfClass(dest, classAlien())) {
		totalLength = longAt((void *)(dest + BaseHeaderSize));
		destAddr = ((/* startOfData:withSize: */
	(totalLength > 0
		? (dest + BaseHeaderSize) + BytesPerOop
		: longAt((void *)((dest + BaseHeaderSize) + BytesPerOop)))) + destStart) - 1;
		if (totalLength) {
...
primAlienReplace(void)
{
	...
	if (isKindOfClass(array, classAlien())) {
		totalLength = longAt((void *)(array + BaseHeaderSize));
		dest = ((/* startOfData:withSize: */
	(totalLength > 0
		? (array + BaseHeaderSize) + BytesPerOop
		: longAt((void *)((array + BaseHeaderSize) + BytesPerOop)))) + start) - 1;
		if (totalLength) {
			totalLength = SQABS(totalLength);
		}
		else {
	...
	if (isKindOfInteger(repl)) {
		...
		if (isKindOfClass(repl, classAlien())) {
			totalLength = longAt((void *)(repl + BaseHeaderSize));
			src = ((/* startOfData:withSize: */
	(totalLength > 0
		? (repl + BaseHeaderSize) + BytesPerOop
		: longAt((void *)((repl + BaseHeaderSize) + BytesPerOop)))) + replStart) - 1;
			if (totalLength) {
				totalLength = SQABS(totalLength);
			}
			else {
...
primAllocateExecutablePage(void)
{
	...
	if (theClassAlien) {
		...
		ptr = firstIndexableField(result);
		ptr[0] = (0 - byteSize);
...
#if SPURVM
		memInteger = /* positiveMachineIntegerFor: */
				(BytesPerWord == 8
...
primDoubleAt(void)
{
	...
	if (!((startAddr = /* startOfData: */
				((longAt((void *)(rcvr + BaseHeaderSize))) > 0
					? (rcvr + BaseHeaderSize) + BytesPerOop
					: longAt((void *)((rcvr + BaseHeaderSize) + BytesPerOop)))))) {
		return primitiveFailFor(PrimErrBadReceiver);
	}
...
primInLibraryFindSymbol(void)
{
	...
	address = ioLoadSymbolOfLengthFromModule(((sqInt) (firstIndexableField(functionName))), byteSizeOf(functionName), ((void *)(longAt((void *)((libraryProxy + BaseHeaderSize) + BytesPerOop)))));
	if ((failed())
	 || (address == 0)) {
...
primReturnFromContextThrough(void)
{
	...
	/* C needs a typedef for structs to be assigned, but that implies a struct class for just one assignment. */
...
primSignedByteAt(void)
{
	...
	value = byteAt((void *)(startAddr + byteOffset));
...
primSignedByteAtPut(void)
{
	...
	byteAtput((void *)(startAddr + byteOffset),value);
	return methodReturnValue(valueOop);
}
...
primSignedLongAt(void)
{
	...
	value = long32At((void *)(startAddr + byteOffset));
...
primSignedLongAtPut(void)
{
	...
	long32Atput((void *)(startAddr + byteOffset),value);
	return methodReturnValue(valueOop);
}
...
primSignedShortAt(void)
{
	...
	value = shortAt((void *)(startAddr + byteOffset));
	return methodReturnInteger(value);
}
...
primSignedShortAtPut(void)
{
	...
	shortAtput((void *)(startAddr + byteOffset),value);
	return methodReturnValue(valueOop);
}
...
primSizeField(void)
{
	...
	valueOop = (BytesPerWord == 8
				? signed64BitIntegerFor(((sqLong) (longAt((void *)(rcvr + BaseHeaderSize)))))
				: signed32BitIntegerFor(((int) (longAt((void *)(rcvr + BaseHeaderSize))))));
	return methodReturnValue(valueOop);
}
...
primSizeFieldPut(void)
{
	...
	if (BytesPerWord == 8) {
		longAtput((void *)(rcvr + BaseHeaderSize),((usqInt) value));
	}
	else {
		longAtput((void *)(rcvr + BaseHeaderSize),((usqInt) value));
	}
...
primStrlenFromStartIndex(void)
{
	...
	limit = longAt((void *)(rcvr + BaseHeaderSize));
	ptr = ((char *) ((/* startOfData:withSize: */
	(limit > 0
		? (rcvr + BaseHeaderSize) + BytesPerOop
		: longAt((void *)((rcvr + BaseHeaderSize) + BytesPerOop)))) + byteOffset));
	if (limit) {
		limit = SQABS(limit);
		index = 0;
		while ((index < limit)
...
primStrlenThroughPointerAtIndex(void)
{
	...
	((longAt((void *)(rcvr + BaseHeaderSize))) > 0
		? (rcvr + BaseHeaderSize) + BytesPerOop
		: longAt((void *)((rcvr + BaseHeaderSize) + BytesPerOop)))) + byteOffset;
	return methodReturnInteger(strlen(((char *) (longAt((void *)(addr))))));
}
...
primUnsignedWordAt(void)
{
	...
	value = longAt((void *)(startAddr + byteOffset));
	return methodReturnValue(/* positiveMachineIntegerFor: */
		(BytesPerWord == 8
			? positive64BitIntegerFor(value)
			: positive32BitIntegerFor(value)));
...
primUnsignedWordAtPut(void)
{
	...
	longAtput((void *)(startAddr + byteOffset),value);
	return methodReturnValue(valueOop);
}
...
{
	return ((longAt((void *)(alienOop + BaseHeaderSize))) > 0
			? (alienOop + BaseHeaderSize) + BytesPerOop
			: longAt((void *)((alienOop + BaseHeaderSize) + BytesPerOop)));
}
