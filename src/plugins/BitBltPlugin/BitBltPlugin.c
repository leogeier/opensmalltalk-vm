/* Automatically generated by
	SmartSyntaxPluginCodeGenerator VMMaker.oscog-eem.3723 uuid: 04f446ec-32a3-400b-89bd-7238de59f55e
	(Compiler-eem.527)
   from
	BitBltSimulation VMMaker.oscog-eem.3723 uuid: 04f446ec-32a3-400b-89bd-7238de59f55e
 */
static char __buildInfo[] = "BitBltSimulation VMMaker.oscog-eem.3723 uuid: 04f446ec-32a3-400b-89bd-7238de59f55e " __DATE__ ;
...
static unsigned int alphaBlendUnscaledwith(unsigned int sourceWord, unsigned int destinationWord);
...
static unsigned int alphaScalewith(unsigned int sourceWord, unsigned int destinationWord);
static sqInt alphaSourceBlendBits16(void);
static sqInt alphaSourceBlendBits32(void);
static sqInt alphaSourceBlendBits8(void);
static unsigned int alphaUnscalewith(unsigned int sourceWord, unsigned int destinationWord);
...
#endif
static sqInt (*isOopMutable)(sqInt anOop);
...
static sqIntptr_t (*lockSurfaceFn)(sqIntptr_t, sqInt *, sqInt, sqInt, sqInt, sqInt);
static sqInt mask1;
static sqInt mask2;
...
static const char *moduleName = "BitBltPlugin VMMaker.oscog-eem.3723 " INT_EXT;
...
static int (*querySurfaceFn)(sqIntptr_t, sqInt *, sqInt *, sqInt *, sqInt *);
static sqInt skew;
...
static int (*unlockSurfaceFn)(sqIntptr_t, sqInt, sqInt, sqInt, sqInt);
static sqInt vDir;
...
alphaBlendConstwithpaintMode(unsigned int sourceWord, unsigned int destinationWord, sqInt paintMode)
{
	...
	if (destPPW == 1) {
		...
		if (!(paintMode && (sourceWord == 0))) {
			/* blend red and blue */
			blendRB = (((sourceWord & 0xFF00FF) * sourceAlpha) + ((destinationWord & 0xFF00FF) * unAlpha)) + 0x800080;

			/* blend alpha and green */
			blendAG = ((((((usqInt)(sourceWord)) >> 8) & 0xFF00FF) * sourceAlpha) + (((((usqInt)(destinationWord)) >> 8) & 0xFF00FF) * unAlpha)) + 0x800080;

			/* divide by 255 */
			blendRB = (((usqInt)((((((usqInt)(blendRB)) >> 8) & 0xFF00FF) + blendRB))) >> 8) & 0xFF00FF;
			blendAG = (((usqInt)((((((usqInt)(blendAG)) >> 8) & 0xFF00FF) + blendAG))) >> 8) & 0xFF00FF;
		...
		for (j = 1; j <= destPPW; j += 1) {
			...
			if (!(((maskShifted & pixMask) == 0)
				...
				for (i = 1; i <= 3; i += 1) {
					...
					/* +128 for rounding */
					blend = ((((((usqInt)(sourcePixVal)) >> shift) & rgbMask) * sourceAlpha) + (((((usqInt)(destPixVal)) >> shift) & rgbMask) * unAlpha)) + 128;

					/* divide by 255 */
					blend = (((usqInt)(((((usqInt)(blend)) >> 8) + blend))) >> 8) & rgbMask;
...
static unsigned int
alphaBlendUnscaledwith(unsigned int sourceWord, unsigned int destinationWord)
{
	unsigned int alpha;
	unsigned int blendA;
	unsigned int blendB;
	...
	blendA = ((0xFF * alpha) + ((0xFF - alpha) * (((usqInt)(destinationWord)) >> 24))) + 128;

	/* divide by 255 */
	blendA = (((usqInt)(((((usqInt)(blendA)) >> 8) + blendA))) >> 8) & 0xFF;

	/* blend red */
	blendR = (((((sourceWord & 0xFF0000) * alpha) + ((destinationWord & 0xFF0000) * (blendA - alpha))) + ((((usqInt)(blendA) << 15)))) / blendA) & 0xFF0000;

	/* blend green */
	blendG = (((((sourceWord & 0xFF00) * alpha) + ((destinationWord & 0xFF00) * (blendA - alpha))) + ((((usqInt)(blendA) << 7)))) / blendA) & 0xFF00;

	/* blend blue */
	blendB = (((((sourceWord & 0xFF) * alpha) + ((destinationWord & 0xFF) * (blendA - alpha))) + (((usqInt)(blendA)) >> 1)) / blendA) & 0xFF;
...
alphaBlendwith(unsigned int sourceWord, unsigned int destinationWord)
{
	...
	/* blend red and blue */
	blendRB = (((sourceWord & 0xFF00FF) * alpha) + ((destinationWord & 0xFF00FF) * unAlpha)) + 0xFF00FF;

	/* blend alpha and green */
	blendAG = (((((((usqInt)(sourceWord)) >> 8) | 0xFF0000) & 0xFF00FF) * alpha) + (((((usqInt)(destinationWord)) >> 8) & 0xFF00FF) * unAlpha)) + 0xFF00FF;

	/* divide by 255 */
	blendRB = (((usqInt)((blendRB + ((((usqInt)((blendRB - 0x10001))) >> 8) & 0xFF00FF)))) >> 8) & 0xFF00FF;
	blendAG = (((usqInt)((blendAG + ((((usqInt)((blendAG - 0x10001))) >> 8) & 0xFF00FF)))) >> 8) & 0xFF00FF;
	result = blendRB | ((((usqInt)(blendAG) << 8)));
	return result;
}
...
static unsigned int
alphaScalewith(unsigned int sourceWord, unsigned int destinationWord)
{
	unsigned int alpha;
	unsigned int g;
	unsigned int rb;
	...
	/* divide by 255 */
	rb = (((usqInt)((rb + ((((usqInt)((rb - 0x10001))) >> 8) & 0xFF00FF)))) >> 8) & 0xFF00FF;

	/* scale green component */
	g = ((destinationWord & 0xFF00) * alpha) + 0x8000;

	/* divide by 255 */
	g = (((usqInt)((g + ((((usqInt)((g - 0x100))) >> 8) & 0xFF00)))) >> 8) & 0xFF00;
	...
	while (((deltaY -= 1)) != 0) {
		...
		while (((deltaX -= 1)) != 0) {
			...
			if (srcAlpha == 0xFF) {
				...
				sourceWord = ((unsigned int) (((((((usqInt)((dither8Lookup[addThreshold + (((((usqInt)(sourceWord)) >> 16)) & 0xFF)])) << 10)))) + (((((usqInt)((dither8Lookup[addThreshold + (((((usqInt)(sourceWord)) >> 8)) & 0xFF)])) << 5))))) + (dither8Lookup[addThreshold + (sourceWord & 0xFF)])));
				if (sourceWord) {
				...
				if (srcAlpha) {
					...
					sourceWord = ((unsigned int) (((((((usqInt)((dither8Lookup[addThresholdSqInt + (((((usqInt)(sourceWord)) >> 16)) & 0xFF)])) << 10)))) + (((((usqInt)((dither8Lookup[addThresholdSqInt + (((((usqInt)(sourceWord)) >> 8)) & 0xFF)])) << 5))))) + (dither8Lookup[addThresholdSqInt + (sourceWord & 0xFF)])));
					if (sourceWord) {
						sourceWord = (((usqInt)(sourceWord) << srcShift));
					}
					else {
			...
			if (srcAlpha == 0xFF) {
				...
				while ((((deltaX -= 1)) != 0)
				 && ((((usqInt)(((/* begin srcLongAt: */
					assert((((usqInt)srcIndex)) < endOfSource),
				(sourceWord = long32At((void *)(srcIndex))))))) >> 24) == 0xFF)) {
					/* dstLongAt:put: */
					long32Atput((void *)(dstIndex),sourceWord);
					srcIndex += 4;
					dstIndex += 4;
				...
				if (srcAlpha) {
					...
					while ((((deltaX -= 1)) != 0)
					 && ((((usqInt)(((/* begin srcLongAt: */
						assert((((usqInt)srcIndex)) < endOfSource),
					(sourceWord = long32At((void *)(srcIndex))))))) >> 24) == 0)) {
						srcIndex += 4;
						dstIndex += 4;
					}
...
alphaSourceBlendBits8(void)
{
	...
	assert((((usqInt)srcIndex)) < endOfSource),
long32At((void *)(srcIndex)))) & ((unsigned int)~adjust)) + adjust;
			srcAlpha = ((usqInt)(sourceWord)) >> 24;
			if (srcAlpha > 0x1F) {
...
static unsigned int
alphaUnscalewith(unsigned int sourceWord, unsigned int destinationWord)
{
	unsigned int alpha;
	unsigned int b;
	unsigned int carry;
	...
	r = (((destinationWord & 0xFF0000) * 0xFF) + ((((usqInt)((alpha + 1)) << 15)))) / alpha;

	/* unscale green component */
	g = (((destinationWord & 0xFF00) * 0xFF) + ((((usqInt)((alpha + 1)) << 7)))) / alpha;

	/* unscale blue components */
	b = (((destinationWord & 0xFF) * 0xFF) + (((usqInt)((alpha + 1))) >> 1)) / alpha;
	carry = ((usqInt)((((r & 0xFF000000U) | (g & 0xFF0000)) | (b & 0xFF00)))) >> 8;
	carry = carry | ((((usqInt)((carry & 0xAAAAAA))) >> 1) | ((((usqInt)((carry & 0x555555)) << 1))));
	carry = carry | ((((usqInt)((carry & 0xCCCCCC))) >> 2) | ((((usqInt)((carry & 0x333333)) << 2))));
	carry = carry | ((((usqInt)((carry & 0xF0F0F0))) >> 4) | ((((usqInt)((carry & 0xF0F0F)) << 4))));
...
l1:
	...
	hDir = (vDir = 1);
	assert(((destBits + (dy * destPitch)) + ((dx / destPPW) * 4)) < endOfDestination);
...
copyLoop(void)
{
 ...
 && (skew == 0))));
	...
	if (noHalftone) {
		...
		halftoneWord = long32At((void *)(halftoneBase + ((0 % halftoneHeight) * 4)));
	}
	y = dy;
	...
	if (combinationRule == 3) {
		for (i = 1; i <= bbH; i += 1) {
			if (halftoneHeight > 1) {
				halftoneWord = long32At((void *)(halftoneBase + ((y % halftoneHeight) * 4)));
				y += vDir;
			}
			...
			if (nWords > 1) {
				destMask = mask2;
				thisWord = ((((skew < 0) ? (((usqInt)(skewMask)) >> (-skew)) : ((((usqInt)(skewMask) << skew))))) & mask2
							? (/* begin srcLongAt: */
								assert((((usqInt)sourceIndex)) < endOfSource),
							long32At((void *)(sourceIndex)))
							: /* we don't need more bits, they will all come from prevWord */ 0);
...
copyLoopNoSource(void)
{
	...
	for (i = 1; i <= bbH; i += 1) {
		if (!noHalftone) {
			halftoneWord = long32At((void *)(halftoneBase + ((((dy + i) - 1) % halftoneHeight) * 4)));
		}
		...
		do {
			...
			if (mapperFlags == (ColorMapPresent | ColorMapIndexedPart)) {
				...
				do {
					...
					skewWord = skewWord | ((((usqInt)((destPix & destPixMask)) << dstShiftSqInt)));
...
loadBitBltFromwarping(sqInt bbObj, sqInt aBool)
{
	...
	/* begin fetchInteger:ofObject:ifFail: */
	fieldOop = fetchPointerofObject(FormWidthIndex, destForm);
	if (isIntegerObject(fieldOop)) {
		destWidth = integerValueOf(fieldOop);
	}
	else {
		return 0;
	}
	...
	/* begin fetchInteger:ofObject:ifFail: */
	fieldOop = fetchPointerofObject(FormHeightIndex, destForm);
	if (isIntegerObject(fieldOop)) {
		destHeight = integerValueOf(fieldOop);
	}
	else {
		return 0;
	}
	...
	/* begin fetchInteger:ofObject:ifFail: */
	fieldOop = fetchPointerofObject(FormDepthIndex, destForm);
	if (isIntegerObject(fieldOop)) {
		destDepth = integerValueOf(fieldOop);
	}
	else {
		return 0;
	}
	...
	if (isIntegerObject(destBits)) {
		...
		if (!((isWordsOrBytes(destBits))
			 && ((byteSizeOf(destBits)) >= (destPitch * destHeight)))) {
			return 0;
		}
	...
	/* begin fetchIntOrFloat:ofObject:ifNil:ifFail: */
	fieldOop = fetchPointerofObject(BBDestXIndex, bitBltOop);
	if (isIntegerObject(fieldOop)) {
		destX = integerValueOf(fieldOop);
	}
	else {
		if ((isFloatObject(fieldOop))
		 && ((-2.147483648e9 <= ((floatValue = floatValueOf(fieldOop))))
		 && (floatValue <= 2.147483647e9))) {
			destX = ((sqInt)floatValue);
		}
		else {
	...
	/* begin fetchIntOrFloat:ofObject:ifNil:ifFail: */
	fieldOop = fetchPointerofObject(BBDestYIndex, bitBltOop);
	if (isIntegerObject(fieldOop)) {
		destY = integerValueOf(fieldOop);
	}
	else {
		if ((isFloatObject(fieldOop))
		 && ((-2.147483648e9 <= ((floatValue = floatValueOf(fieldOop))))
		 && (floatValue <= 2.147483647e9))) {
			destY = ((sqInt)floatValue);
		}
		else {
			if (fieldOop == (nilObject())) {
				destY = 0;
	...
	/* begin fetchIntOrFloat:ofObject:ifNil:ifFail: */
	fieldOop = fetchPointerofObject(BBWidthIndex, bitBltOop);
	if (isIntegerObject(fieldOop)) {
		width = integerValueOf(fieldOop);
	}
	else {
		if ((isFloatObject(fieldOop))
		 && ((-2.147483648e9 <= ((floatValue = floatValueOf(fieldOop))))
		 && (floatValue <= 2.147483647e9))) {
			width = ((sqInt)floatValue);
		}
		else {
			if (fieldOop == (nilObject())) {
				width = destWidth;
	...
	/* begin fetchIntOrFloat:ofObject:ifNil:ifFail: */
	fieldOop = fetchPointerofObject(BBHeightIndex, bitBltOop);
	if (isIntegerObject(fieldOop)) {
		height = integerValueOf(fieldOop);
	}
	else {
		if ((isFloatObject(fieldOop))
		 && ((-2.147483648e9 <= ((floatValue = floatValueOf(fieldOop))))
		 && (floatValue <= 2.147483647e9))) {
			height = ((sqInt)floatValue);
		}
		else {
			if (fieldOop == (nilObject())) {
				height = destHeight;
	...
	if (noSource) {
		...
		/* begin loadBitBltSourceFormIfFail: */
		...
		/* begin fetchIntOrFloat:ofObject:ifFail: */
		...
		/* begin fetchInteger:ofObject:ifFail: */
		fieldOop = fetchPointerofObject(FormDepthIndex, sourceForm);
		if (isIntegerObject(fieldOop)) {
			sourceDepth = integerValueOf(fieldOop);
		}
		else {
			return 0;
		}
		...
		if (isIntegerObject(sourceBits)) {
			...
			if (!((isWordsOrBytes(sourceBits))
				 && ((byteSizeOf(sourceBits)) >= (sourcePitch * sourceHeight)))) {
				return 0;
			}
		...
		/* end loadBitBltSourceFormIfFail: */
l1:

		/* begin loadColorMapIfFail: */
		...
		if (isWords(cmOop)) {
			...
			/* begin loadColorMapShiftOrMaskFrom:type:ifFail: */
			if (mapOop == (nilObject())) {
				cmShiftTable = null;
			}
			else {
...
l5:
		...
		/* begin fetchIntOrFloat:ofObject:ifNil:ifFail: */
		fieldOop = fetchPointerofObject(BBSourceXIndex, bitBltOop);
		if (isIntegerObject(fieldOop)) {
			sourceX = integerValueOf(fieldOop);
		}
		else {
			if ((isFloatObject(fieldOop))
			 && ((-2.147483648e9 <= ((floatValue = floatValueOf(fieldOop))))
			 && (floatValue <= 2.147483647e9))) {
				sourceX = ((sqInt)floatValue);
			}
			else {
				if (fieldOop == (nilObject())) {
					sourceX = 0;
		...
		/* begin fetchIntOrFloat:ofObject:ifNil:ifFail: */
		fieldOop = fetchPointerofObject(BBSourceYIndex, bitBltOop);
		if (isIntegerObject(fieldOop)) {
			sourceY = integerValueOf(fieldOop);
		}
		else {
			if ((isFloatObject(fieldOop))
			 && ((-2.147483648e9 <= ((floatValue = floatValueOf(fieldOop))))
			 && (floatValue <= 2.147483647e9))) {
				sourceY = ((sqInt)floatValue);
			}
			else {
				if (fieldOop == (nilObject())) {
					sourceY = 0;
...
l6:
	...
	/* begin fetchIntOrFloat:ofObject:ifNil:ifFail: */
	fieldOop = fetchPointerofObject(BBClipXIndex, bitBltOop);
	if (isIntegerObject(fieldOop)) {
		clipX = integerValueOf(fieldOop);
	}
	else {
		if ((isFloatObject(fieldOop))
		 && ((-2.147483648e9 <= ((floatValue = floatValueOf(fieldOop))))
		 && (floatValue <= 2.147483647e9))) {
			clipX = ((sqInt)floatValue);
		}
		else {
			if (fieldOop == (nilObject())) {
				clipX = 0;
	...
	/* begin fetchIntOrFloat:ofObject:ifNil:ifFail: */
	fieldOop = fetchPointerofObject(BBClipYIndex, bitBltOop);
	if (isIntegerObject(fieldOop)) {
		clipY = integerValueOf(fieldOop);
	}
	else {
		if ((isFloatObject(fieldOop))
		 && ((-2.147483648e9 <= ((floatValue = floatValueOf(fieldOop))))
		 && (floatValue <= 2.147483647e9))) {
			clipY = ((sqInt)floatValue);
		}
		else {
			if (fieldOop == (nilObject())) {
				clipY = 0;
	...
	/* begin fetchIntOrFloat:ofObject:ifNil:ifFail: */
	fieldOop = fetchPointerofObject(BBClipWidthIndex, bitBltOop);
	if (isIntegerObject(fieldOop)) {
		clipWidth = integerValueOf(fieldOop);
	}
	else {
		if ((isFloatObject(fieldOop))
		 && ((-2.147483648e9 <= ((floatValue = floatValueOf(fieldOop))))
		 && (floatValue <= 2.147483647e9))) {
			clipWidth = ((sqInt)floatValue);
		}
		else {
			if (fieldOop == (nilObject())) {
				clipWidth = destWidth;
	...
	/* begin fetchIntOrFloat:ofObject:ifNil:ifFail: */
	fieldOop = fetchPointerofObject(BBClipHeightIndex, bitBltOop);
	if (isIntegerObject(fieldOop)) {
		clipHeight = integerValueOf(fieldOop);
	}
	else {
		if ((isFloatObject(fieldOop))
		 && ((-2.147483648e9 <= ((floatValue = floatValueOf(fieldOop))))
		 && (floatValue <= 2.147483647e9))) {
			clipHeight = ((sqInt)floatValue);
		}
		else {
			if (fieldOop == (nilObject())) {
				clipHeight = destHeight;
			}
			else {
				return 0;
			}
...
OLDtallyIntoMapwith(sqInt sourceWord, sqInt destinationWord)
{
	...
	if (destDepth == 16) {
		...
		if (((d = cmBitsPerColor - 5)) > 0) {
			...
			mapIndex = (destPix + (srcPix & mask)) + (((((usqInt)(srcPix) << d))) & ((((usqInt)(mask) << cmBitsPerColor))));
		}
		else {
			...
			destPix = (destPix + (srcPix & mask)) + ((((usqInt)(srcPix)) >> d) & ((((usqInt)(mask) << cmBitsPerColor))));
			if (!destPix) {
				mapIndex = 1;
				goto l1;
			}
...
static unsigned int
partitionedMulwithnBitswordBits(unsigned int word1, unsigned int word2, sqInt nBits, sqInt wordBits)
{
	...
	/* form a word mask with alternate nBits 0 and nBits 1, so as to select even channels */
	groupMask = channelMask;
	for (i = 2; i <= nGroups; i += 1) {
		groupMask = ((((usqInt)(groupMask) << n2))) + channelMask;
	}

	/* shift for putting odd channels in high-word - usually wordBits, except if wordBits \\ nBits ~= 0 */
	highWordShift = nGroups * n2;

	/* select odd channel interleaved with groups of nBits zeros, so as to leave room for double-precision multiplication */
	...
	for (i = 1; i <= (wordBits / nBits); i += 1) {
		doubleWordMul += ((((unsigned long long) (((((doubleWord1) >> shift) & channelMask) * (((doubleWord2) >> shift) & channelMask)) + half))) << shift);

		/* multiply each channel of the two operands.
		   N.B. we must explicitly cast the expression to be shifted to 8 bytes, otherwise Slang
		   will generate a 32-bit shift, truncating the result and hence breaking the code. */
		shift += n2;
	}
	...
	/* divide by scale */
	doubleWordMul = ((((((doubleWordMul) >> nBits) & doubleGroupMask) + doubleWordMul)) >> nBits) & doubleGroupMask;

	/* compact channels back into a single word */
	result = ((doubleWordMul) >> (highWordShift - nBits)) + (doubleWordMul & groupMask);
	return result;
}
...
{
	if (loadBitBltFrom(stackValue(methodArgumentCount()))) {
		copyBits();
		if (!(failed())) {
			/* begin showDisplayBits */
			/* begin ensureDestAndSourceFormsAreValid */
			...
			if (!(failed())) {
				...
				return methodReturnInteger(bitCount);
			}
		}
	}
	if (!(failed())) {
...
primitiveDrawLoop(void)
{
	...
	if ((isIntegerObject((xDelta = stackValue(1))))
	 && ((isIntegerObject((yDelta = stackValue(0))))
	 && (loadBitBltFrom(stackValue(2))))) {
		xDelta = integerValueOf(xDelta);
		yDelta = integerValueOf(yDelta);
...
primitivePixelValueAtPut(void)
{
	...
	word = ((word | ((((usqInt)(mask) << shift)))) - ((((usqInt)(mask) << shift)))) + ((((usqInt)(pixel) << shift)));
...
primitiveWarpBits(void)
{
	...
	if (!(loadWarpBltFrom(stackValue(methodArgumentCount())))) {
		return primitiveFailFor(PrimErrBadReceiver);
	}
...
rgbAddwith(unsigned int sourceWord, unsigned int destinationWord)
{
	...
	if (destDepth == 16) {
		...
		return partitionedAddtonBitscomponentMaskcarryOverflowMask(sourceWord & 0x7FFF7FFF, destinationWord & 0x7FFF7FFF, 5, componentMask, carryOverflowMask);
	}
	else {
		...
		while (((deltaX -= 1)) != 0) {
			...
			if (srcAlpha) {
				...
				while ((((deltaX -= 1)) != 0)
				 && ((((/* begin srcLongAt: */
					assert((((usqInt)srcIndex)) < endOfSource),
				(sourceWord = long32At((void *)(srcIndex))))) & 0xFFFFFF) == 0)) {
					srcIndex += 4;
					dstIndex += 4;
				}
...
rgbComponentAlpha32with(sqInt sourceWord, sqInt destinationWord)
{
	...
	answer = ((((usqInt)((((((usqInt)((((((usqInt)(a) << 8))) + r)) << 8))) + g)) << 8))) + b;
	return answer;
}
	...
	for (i = 1; i <= destPPW; i += 1) {
		...
		if (!(destDepth == 32)) {
			...
			if (((d = destDepth - 32)) > 0) {
				...
				srcPix = ((sqInt)((usqInt)(srcPix) << d));
				v = (destPix + (srcPix & maskSqInt)) + (((((usqInt)(srcPix) << d))) & ((((usqInt)(maskSqInt) << destDepth))));
			}
			else {
				/* Compress to fewer bits by truncation */
				if (!d) {
				...
				maskSqInt = ((sqInt)((usqInt)(maskSqInt) << destDepth));
				srcPix = ((usqInt)(srcPix)) >> d;
				destPix = (destPix + (srcPix & maskSqInt)) + ((((usqInt)(srcPix)) >> d) & ((((usqInt)(maskSqInt) << destDepth))));
				if (!destPix) {
					v = 1;
					goto l1;
				}
...
rgbMapfromto(sqInt sourcePixel, sqInt nBitsIn, sqInt nBitsOut)
{
	...
	if (((d = nBitsOut - nBitsIn)) > 0) {
		...
		srcPix = ((sqInt)((usqInt)(srcPix) << d));
		return (destPix + (srcPix & mask)) + (((((usqInt)(srcPix) << d))) & ((((usqInt)(mask) << nBitsOut))));
	}
	else {
		/* Compress to fewer bits by truncation */
		if (!d) {
		...
		mask = ((sqInt)((usqInt)(mask) << nBitsOut));
		srcPix = ((usqInt)(srcPix)) >> d;
		destPix = (destPix + (srcPix & mask)) + ((((usqInt)(srcPix)) >> d) & ((((usqInt)(mask) << nBitsOut))));
		if (!destPix) {
			return 1;
		}
		return destPix;
...
rgbMulwith(unsigned int sourceWord, unsigned int destinationWord)
{
	if (destDepth < 16) {
		...
		return partitionedMulwithnBitswordBits(sourceWord, destinationWord, destDepth, 32);
	}
	if (destDepth == 16) {
		/* Mul RGB components of each pixel separately */
		return (partitionedMulwithnBitswordBits(sourceWord & 0xFFFF, destinationWord & 0xFFFF, 5, 16)) + ((((usqInt)((partitionedMulwithnBitswordBits(((usqInt)(sourceWord)) >> 16, ((usqInt)(destinationWord)) >> 16, 5, 16))) << 16)));
	}
	else {
...
#endif
		isOopMutable = interpreterProxy->isOopMutable;
...
l1:;
					...
					if (((dSqInt = cmBitsPerColor - 8)) > 0) {
						...
						mapIndex = (destPixSqInt + (srcPixSqInt & maskSqInt)) + (((((usqInt)(srcPixSqInt) << dSqInt))) & ((((usqInt)(maskSqInt) << cmBitsPerColor))));
					}
					else {
						/* Compress to fewer bits by truncation */
						if (!dSqInt) {
						...
						destPixSqInt = (destPixSqInt + (srcPixSqInt & maskSqInt)) + ((((usqInt)(srcPixSqInt)) >> dSqInt) & ((((usqInt)(maskSqInt) << cmBitsPerColor))));
						if (!destPixSqInt) {
							mapIndex = 1;
							goto l2;
						}
...
/*	This version of the inner loop traverses an arbirary quadrilateral
	source, thus producing a general affine transformation. Answer if
	setup and the operation succeed. */
...
l4:
	...
	for (iSqInt = 0; iSqInt <= warpAlignMask; iSqInt += 1) {
		if (sourceMSB) {
			warpBitShiftTable[iSqInt] = (32 - ((((usqInt)((iSqInt + 1)) << warpSrcShift))));
		}
		else {
...
l7:
						destPix = cmLookupTable[sourcePix & cmMask];
						destWordSqInt = destWordSqInt | ((((usqInt)((destPix & dstMask)) << dstBitShift)));
	...
	do {
		...
		do {
			...
			do {
				...
				srcIndex = (sourceBits + (ySqInt * sourcePitch)) + ((((usqInt)(xSqInt)) >> warpAlignShift) * 4);
...
l1:
		...
		if ((nPix == 0)
		 ...
		 && (nPix < ((n * n) / 2)))) {
			...
			rgb = ((((((usqInt)(a) << 24))) + ((((usqInt)(r) << 16)))) + ((((usqInt)(g) << 8)))) + b;
...
void* BitBltPlugin_exports[][3] = {
	...
	{(void*)_m, "primitiveCompareColors\000\001\000", (void*)primitiveCompareColors},
	{(void*)_m, "primitiveCopyBits\000\002\000", (void*)primitiveCopyBits},
...
EXPORT(signed short) primitiveCopyBitsMetadata = 0x200;
